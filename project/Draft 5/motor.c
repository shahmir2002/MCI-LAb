...
void initialize_encoder(void) {
    ...
    GPIOB->DIR &= ~((1 << ENCODER_A) | (1 << ENCODER_B));
    GPIOB->DEN |= (1 << ENCODER_A) | (1 << ENCODER_B);
    GPIOB->AFSEL |= (1 << ENCODER_A) | (1 << ENCODER_B);
    GPIOB->PCTL = (GPIOB->PCTL & ~0xFF000000) | 0x77000000; // Configure PB6/PB7 for T0CCP0/T0CCP1
}

// Measure encoder speed
int measure_encoder_speed(void) {
    // Implement encoder speed measurement (e.g., pulse count or timer capture)
    static int last_pulse_count = 0;
    int current_pulse_count = TIMER0->TAR; // Example: Read Timer A value
    int speed = current_pulse_count - last_pulse_count; // Calculate speed (difference)
